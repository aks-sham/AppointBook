...
ResultSet resultset;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
         ...
         try
        {
            ...
            String sqlString = "SELECT * FROM doctor WHERE dusername='"+ s1 +"'AND duserpass='"+ ps +"'";
            resultset=statement.executeQuery(sqlString);
            while(resultset.next())
            {
                userid = resultset.getString("dusername");
                password = resultset.getString("duserpass");
            }
        ...
        if(userid.equals(request.getParameter("username")) && password.equals(request.getParameter("userpass")))
        {
            ...
            response.sendRedirect("DMenu.jsp");
        }
        else 
        {   
